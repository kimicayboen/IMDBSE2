        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    $id = isset($row['members_id']) ? $row['members_id'] : '';
                    ...
                    echo "<tr>
                            ...
                            <td>
                                <form method='post' action='admin.php' style='display:inline-block;'>
                                    <input type='hidden' name='members_id' value='{$id}'>
                                    <input type='hidden' name='name' value='{$name}'>
                                    <input type='hidden' name='email' value='{$email}'>
                                    <input type='hidden' name='type' value='{$membership_type}'>
                                </form>
                                <form method='post' action='admin.php' style='display:inline-block;'>
                                    <input type='hidden' name='member_id' value='{$id}'> <!-- Use member_id, not members_id -->
                                    <button type='submit' name='action' value='reset_password'>Reset</button>
                                </form>
                                <form method='post' action='admin.php' style='display:inline-block;'>
                                    <input type='hidden' name='member_id' value='{$id}'> <!-- Use member_id -->
...
<form method="post" action="admin.php" class="form-container">
    <!-- No member_id input here since it's auto-incremented -->
